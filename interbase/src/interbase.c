	...
	Dbi *db)
{
	...
	if (dbdata->autocommit) {
		isc_rollback_retaining(dbdata->status, &(dbdata->trans));
	}
}
	...
	Tcl_Obj *objv[])
{
	...
	dpbpos = 0;
	if (dpb != NULL) {Tcl_Free(dpb);}
	dbdata->dpblen = 200;
	dpb = Tcl_Alloc(dbdata->dpblen*sizeof(char));
	dpb[dpbpos++] = isc_dpb_version1;
	for (i = 3; i < objc; i++) {
		...
		if (item != -1) {
			if ((dpbpos+len+2) > dbdata->dpblen) {
				...
				dpb = Tcl_Realloc(dpb,dbdata->dpblen*sizeof(char));
			}
			dpb[dpbpos++]=item;
			dpb[dpbpos++]=len;
			strncpy(dpb+dpbpos,string,len);
			dpbpos += len;
		}
	}
	string = Tcl_GetStringFromObj(objv[2],&len);
	error = isc_attach_database(dbdata->status, len, string, &(dbdata->db), dpbpos, dpb);
	if (error) {
	...
	for (i = 0, var = out_sqlda->sqlvar; i < out_sqlda->sqld; i++, var++) {
		...
		switch(dtype) {
			...
			case SQL_INT64:
				...
				break;
			default:
				dbi_Interbase_Free_sqlda(out_sqlda);
	...
	Dbi *db)
{
	...
	if (dbdata->out_sqlda->sqld != 0) {
		...
		dbi_Interbase_Free_sqlda(dbdata->out_sqlda);
	}
	...
	Tcl_Obj **objv)
{
	...
	if (error) {
		Tcl_AppendResult(interp,"Interbase error on preparing", NULL);
		goto error;
	}
...
/*                fprintf(stdout, "%08lx:%08lx\n", blob_id->isc_quad_high, blob_id->isc_quad_low);fflush(stdout);*/
	...
	if (dbdata->out_sqlda->sqld == 0) {
		...
		*/
		...
		if (dbdata->out_sqlda->sqld > dbdata->out_sqlda->sqln) {
			...
			if (error) {
				Tcl_AppendResult(interp,"database error executing", NULL);
				goto error;
			}
		}
		error = dbi_Interbase_Prepare_out_sqlda(interp,dbdata->out_sqlda);
		if (error) {
			Tcl_AppendResult(interp," executing", NULL);
			goto error;
		}
	}
	...
	error:
		...
		Tcl_AppendResult(interp," command: \"",	Tcl_GetStringFromObj(temp,NULL), "\":\n", NULL);
	...
	switch (fetch_option) {
		...
		case DBI_FETCH_LINES:
			if (dbdata->nrows == -1) {
				...
				Tcl_SetObjResult(interp,Tcl_NewIntObj(dbdata->nrows));
				break;
			}
