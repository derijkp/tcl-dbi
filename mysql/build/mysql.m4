...
# SC_MYSQL --
#
#	Locate the installed public mysql files
#
# Arguments:
#	None.
#
...
#	Adds a --with-mysqllib switch to configure.
#	Result is cached.
#
...
AC_DEFUN(SC_MYSQL_INCLUDE, [
	...
	AC_CACHE_VAL(ac_cv_c_mysqlinclude, [
		...
		for i in \
			...
			/usr/local/include/mysql /usr/include/mysql; do
...
AC_DEFUN(SC_MYSQL_LIB, [
    ...
    AC_ARG_WITH(mysqllib, [ --with-mysqllib      directory containing the mysql library files.], with_mysqllib=${withval})
    if test x"${with_mysqllib}" != x ; then
	if test -f "${with_mysqllib}/libmysqlclient.so" ; then
	    ac_cv_c_mysqllib=${with_mysqllib}
	elif test -f "${with_mysqllib}/libmysqlclient.a" ; then
	    ac_cv_c_mysqllib=${with_mysqllib}
	elif test -f "${with_mysqllib}/mysql.lib" ; then
	    ac_cv_c_mysqllib=${with_mysqllib}
	else
	    AC_MSG_ERROR([${with_mysqllib} directory does not contain mysql public library file $libmysqlfile])
	fi
    else
	AC_CACHE_VAL(ac_cv_c_mysqllib, [
	    # Use the value from --with-mysqllib, if it was given
	...
	case "`uname -s`" in
		*win32* | *WIN32* | *CYGWIN_NT* |*CYGWIN_98*|*CYGWIN_95*)
		    if test x"${ac_cv_c_mysqllib}" = x ; then
			AC_MSG_ERROR(mysql.lib not found.  Please specify its location with --with-mysqllib)
		    else
			AC_MSG_RESULT(${ac_cv_c_mysqllib})
		    fi
		
		    # Convert to a native path and substitute into the output files.
		
		    MYSQL_LIB=\"`${CYGPATH} "${ac_cv_c_mysqllib}/mysql_ms.lib"`\"
		;;
		*)
		    if test x"${ac_cv_c_mysqllib}" = x ; then
			AC_MSG_ERROR(libmysqlclient.so or libmysqlclient.a not found.  Please specify its location with --with-mysqllib)
		    else
			AC_MSG_RESULT(${ac_cv_c_mysqllib})
		    fi
		    ...
		    LIB_DIR_NATIVE=`${CYGPATH} "${ac_cv_c_mysqllib}"`
